...
serve(async (req) => {
  ...
  try {
    const { chamado_id, to, subject, body, original_ai_response, force = false } = await req.json();
    ...
    const { data: chamado } = await supabase.from("chamados")
      .select("thread_id,nivel_risco,motivo_bloqueio_auto,confianca_ia,categoria")
      .eq("id", chamado_id).maybeSingle();
    const { data: settings } = await supabase.from("system_settings").select("*").limit(1).maybeSingle();

    // Validação de risco — só bloqueia se não for forçado por humano
    if (!force && chamado?.nivel_risco === "alto") {
      return new Response(JSON.stringify({
        error: "Resposta bloqueada: chamado de alto risco. Confirme envio manualmente.",
        risk: chamado.nivel_risco, reason: chamado.motivo_bloqueio_auto,
